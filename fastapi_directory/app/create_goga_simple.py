...
def create_goga_user():
    ...
    try:
        ...
        result = db.execute('SELECT id FROM "user" WHERE username = :username', {"username": "Goga"})
        ...
        result = db.execute('SELECT id FROM roles WHERE name = :name', {"name": "Главный администратор"})
        ...
        db.execute(
            'INSERT INTO "user" (username, realname, password_hash, role_id, admin, sadmin) VALUES (:username, :realname, :password_hash, :role_id, :admin, :sadmin)',
            {
                "username": "Goga",
                "realname": "Goga",
